...
async fn main() -> anyhow::Result<()> {
    ...
    let wallet_address_str = std::env::var("WALLET_ADDRESS")
        .unwrap_or_else(|_| "5Q544fKrFoe6tsEbD7S8EmxGTJYAKtTVhAW5Q5pge4j1".to_string());

    let tracking_mode = std::env::var("TRACKING_MODE")
        .unwrap_or_else(|_| "polling".to_string()); // "polling" or "websocket"
    ...
    let data_provider: Arc<dyn PortfolioDataProvider> = if tracking_mode == "websocket" {
        ...
        let on_update = Arc::new(|wallet: Pubkey, balance: TokenBalance| {
            info!("Real-time update for {}: {} {}", 
                wallet, 
                balance.ui_amount, 
                balance.symbol.as_deref().unwrap_or("Unknown")
            );
        });
    ...
    if tracking_mode == "websocket" {
        ...
        // Polling mode
        info!("Polling portfolio tracker is running with interval {}ms. Press Ctrl+C to stop.", tick_interval);
