...
use crate::models::{
  portfolio::{PortfolioDiff, PortfolioSnapshot, TokenChange},
  token::TokenBalance,
};
...
pub struct PortfolioTracker {
  ...
  notification_queue: NotificationQueue, // Replace event_handler with queue
...
impl PortfolioTracker {
  ...
  pub fn new(
    ...
    // Create notification queue with the event handler
    let notification_queue = NotificationQueue::new(event_handler);

    Self {
      wallet_address,
      data_provider,
      price_provider,
      notification_queue, // Use queue instead of direct handler
  ...
  pub fn add_token_event_handler(
    &mut self,
    handler: Arc<dyn TokenEventHandler>,
  ) {
  ...
  pub async fn log_initial_portfolio(&self) -> anyhow::Result<()> {
    ...
    let balances =
      self.data_provider.fetch_token_balances(&self.wallet_address).await?;

    if balances.is_empty() {
      info!("No token holdings found in wallet");

      // Queue empty portfolio notification (non-blocking)
      let empty_diff =
        PortfolioDiff { added: vec![], removed: vec![], changes: vec![] };
      self.notification_queue.notify_portfolio_change(empty_diff);
    ...
    let sol_balance =
      self.data_provider.fetch_sol_balance(&self.wallet_address).await?;
    ...
    // Queue initial portfolio notification (non-blocking)
    ...
    self.notification_queue.notify_portfolio_change(initial_diff);

    Ok(())
  }

  /// Format initial portfolio message for notifications
  async fn format_initial_portfolio_message(
    &self,
    ...
    message
      .push_str(&format!("📊 <b>Total Tokens:</b> {}\n\n", balances.len()));
    ...
    if !balances.is_empty() {
      ...
      for balance in &balance_vec {
        if let Some(price) =
          self.price_provider.get_token_price(&balance.mint).await
        {
      ...
      balance_with_value.sort_by(|a, b| {
        b.0.partial_cmp(&a.0).unwrap_or(std::cmp::Ordering::Equal)
      });

      for (value, balance) in balance_with_value.iter().take(10) {
        // Show top 10
        let symbol = balance.symbol.as_deref().unwrap_or("Unknown");
        if *value > 0.0 {
          message.push_str(&format!(
    ...
    let sol_balance =
      match self.data_provider.fetch_sol_balance(&self.wallet_address).await {
        Ok(bal) => bal,
        Err(_) => 0.0,
      };
    ...
    for (mint, new_balance) in &new.balances {
      match old.balances.get(mint) {
        ...
        None => {
          ...
          // Notify token added handlers (these are fast, keep them in-line)
          for handler in &self.token_event_handlers {
            ...
            tokio::spawn(async move {
              handler_clone
                .on_token_added(&token_address, &balance_clone)
                .await;
            });
          }
        }
    ...
    for (mint, old_balance) in &old.balances {
      if !new.balances.contains_key(mint) {
        ...
        for handler in &self.token_event_handlers {
          ...
          tokio::spawn(async move {
            handler_clone
              .on_token_removed(&token_address, &balance_clone)
              .await;
          });
        }
      }
  ...
  pub async fn start_tracking_polling(
    &self,
    tick_interval_ms: u64,
  ) -> anyhow::Result<()> {
    info!(
      "Starting polling-based tracking with interval: {}ms",
      tick_interval_ms
    );
    ...
    loop {
      ...
      match self.take_snapshot().await {
        Ok(new_snapshot) => {
          ...
          if let Some(old) = old_snapshot {
            ...
            if !diff.is_empty() {
              ...
              self.notification_queue.notify_portfolio_change(diff);
            }
          }
        ...
        Err(e) => {
          ...
          self.notification_queue.notify_error(&e);
        }
      }
    }
  }
...
impl Clone for PortfolioTracker {
  fn clone(&self) -> Self {
    Self {
      ...
      notification_queue: self.notification_queue.clone(), // Clone the queue
